package main.java.Medium; /**https://leetcode.com/problems/binary-tree-coloring-game/ */
...
class TreeNode64 {
      ...
      TreeNode64 right;
      TreeNode64() {}
      TreeNode64(int val) { this.val = val; }
      TreeNode64(int val, TreeNode64 left, TreeNode64 right) {
          this.val = val;
          this.left = left;
          this.right = right;
      }
...
class Solution39 {
    public boolean btreeGameWinningMove(TreeNode64 root, int n, int x) {
    ...
    private int dfs(TreeNode64 root, int[] count, int x) {
        if(root == null) {
            return 0;
        }
