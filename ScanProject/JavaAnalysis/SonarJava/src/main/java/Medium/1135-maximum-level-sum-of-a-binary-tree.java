package main.java.Medium; /**https://leetcode.com/problems/maximum-level-sum-of-a-binary-tree/ */
...
class TreeNode65 {
      ...
      TreeNode65 right;
      TreeNode65() {}
      TreeNode65(int val) { this.val = val; }
...
class Solution42 {
    public int maxLevelSum(TreeNode65 root) {
        ...
        Queue<TreeNode65> queue = new LinkedList<>();
        queue.offer(root);
        while(!queue.isEmpty()) {
            int size = queue.size();
            int sum = 0;
            for(int i = 0; i < size; i++) {
                TreeNode65 curr = queue.poll();
                sum += curr.val;
                if(curr.left != null) {
