package main.java.Medium; /**https://leetcode.com/problems/delete-nodes-and-return-forest/ */
...
class TreeNode62 {
     int val;
      ...
      TreeNode62 right;
      TreeNode62() {}
      TreeNode62(int val) { this.val = val; }
...
class Solution27 {
    public List<TreeNode62> delNodes(TreeNode62 root, int[] to_delete) {
        ...
        List<TreeNode62> list = new ArrayList<>();
        dfs(root, set, list);
    ...
    private TreeNode62 dfs(TreeNode62 root, Set<Integer> set, List<TreeNode62> list) {
        if(root == null) {
            return null;
        }
        if(set.contains(root.val)) {
